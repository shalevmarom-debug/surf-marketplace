...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--surf-border)] bg-[var(--surf-card)] shadow-sm">
      <div className="mx-auto flex max-w-7xl items-center gap-4 px-4 py-3">
        <Link
          href="/"
          className="shrink-0 text-xl font-bold tracking-tight text-[var(--foreground)] hover:opacity-90"
        >
          {APP_NAME}
        </Link>

        <form action="/" method="GET" className="flex-1 max-w-xl mx-auto hidden sm:block">
          <input
            type="search"
            name="q"
            value={searchQ}
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Link
            href="/new-listing"
            className="rounded-xl bg-[var(--surf-primary)] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[var(--surf-primary-hover)] transition-colors"
          >
            Post a board
          </Link>
          {loading ? (
